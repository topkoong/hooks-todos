			...
			<ul className="list-none text-white p-0">
				{state.todos.map(todo => (
					...
					>
						<span
							onDoubleClick={async () => {
								const response = await axios.patch(`https://hooks-api.theeruttop.now.sh/todos/${todo.id}`, {
									complete: !todo.complete
								});
								dispatch({ type: 'TOGGLE_TODO', payload: response.data })
							}}
							className={`flex-1 ml-12 cursor-pointer ${todo.complete &&
								'line-through text-grey-900'}`}
						>
							{todo.text}
						</span>
						<button
						...
						<button
							onClick={async () => {
								await axios.delete(`https://hooks-api.theeruttop.now.sh/todos/${todo.id}`);
